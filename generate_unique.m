function [uniq_possible_sq, uniq_possible_row, uniq_possible_col] = generate_unique(poss)
    uniq_possible_sq = poss;
    uniq_possible_row = poss;
    uniq_possible_col = poss;
    for i = 1:size(poss,1)
        for j = 1:size(poss,2)
            i_low = fix((i-1)/3)*3; i_range = i_low + 1:fix((i-1)/3)*3+3; 
            j_low = fix((j-1)/3)*3; j_range = j_low + 1:fix((j-1)/3)*3+3;
